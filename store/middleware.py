...
class ProfileSetupMiddleware:
    """
    Middleware to ensure users complete their profile setup before accessing
    the site.
    ...
    def __call__(self, request):
        ...
        Returns:
            HttpResponse: Either a redirect to profile setup or the normal
            response
        """
        ...
        if (request.user.is_authenticated and
            ...
            if (not self.is_profile_complete(request.user) and
    ...
    def is_profile_complete(self, user):
        ...
        Notes:
            - For social auth users, checks if temporary username is still
              in use
        ...
        if social_account:
            return (has_basic_info and
                    user.username != f"user_{social_account.uid}")
